  ...
  <div>
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-md">
      <table class="w-full text-sm font-medium text-left text-gray-500 rtl:text-right dark:text-gray-400">
        ...
        <tbody id="listproxi">
          <tr id="notfoundp" v-if="infoprox === 'Proximity is off'">
            <td colspan="4" class="p-2 text-base font-semibold text-center text-gray-900 bg-white">
              Belum ada produksi untuk hari ini
            </td>
          </tr>
          <tr v-else v-for="(data, index) in getproxi.data" :key="index" class="bg-white dark:bg-gray-800">
    ...
    <footer
      ...
      <div class="w-full max-w-screen-xl mx-auto md:flex md:items-center md:justify-between">
        <table class="w-full text-left text-gray-500 text-md rtl:text-right dark:text-gray-400">
          <tbody>
            <tr v-if="infoprox === 'Proximity is off'" class="bg-white dark:bg-gray-800">
            ...
            <tr v-else-if="getproxi.data && getproxi.data.length > 0" class="bg-white dark:bg-gray-800">
              <th scope="row" class="px-3 py-4 font-bold text-gray-900 whitespace-nowrap dark:text-white">Jumlah Cycle:
              </th>
              <td class="px-3 py-4 font-bold text-gray-900 whitespace-nowrap" id="jcycle">{{ getproxi.data[0].cycle }}
              </td>
              <td class="px-3 py-4 font-bold text-gray-900 whitespace-nowrap">Jumlah Produk:</td>
              <td class="px-3 py-4 font-bold text-gray-900 whitespace-nowrap" id="jproduk">{{ getproxi.data[0].cycle *
          qtyproduct }}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  computed: {
    ...
    isDisabled() {
      return this.infoprox === 'Proximity is off'
    },
    buttonClass() {
      return {
  ...
  methods: {
    ...mapActions(dataliststore, ['a$proxi', 'a$mesin', 'a$delprox']),
    ...
    async fetchqtymesin() {
      ...
      if (qtyMesinArray.length > 0) {
        this.qtyproduct = qtyMesinArray[0].qty_product;
      }
    },
    async fetchtabproxi() {
      await this.a$proxi();
